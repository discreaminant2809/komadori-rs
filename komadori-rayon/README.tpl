# {{crate}} {{version}}

{{readme}}

[`Vec`]: https://doc.rust-lang.org/std/vec/struct.Vec.html
[`ParallelIterator`]: https://docs.rs/rayon/latest/rayon/iter/trait.ParallelIterator.html
[`ParallelCollector`]: https://docs.rs/{{crate}}/latest/{{crate}}/collector/trait.ParallelCollector.html
[`feed_into()`]: https://docs.rs/{{crate}}/latest/{{crate}}/iter/trait.RayonParallelIteratorExt.html#method.feed_into
[max_vec_bench_mark]: https://github.com/discreaminant2809/komadori-rs/blob/main/komadori-rayon/benches/max_vec.rs